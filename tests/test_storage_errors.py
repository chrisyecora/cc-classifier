...
def test_append_transactions_conditional_failure(mocker, env_setup):
    ...
    mock_dynamodb_client.exceptions.TransactionCanceledException = boto3.client(
        "dynamodb", region_name="us-east-1"
    ).exceptions.TransactionCanceledException
    mock_dynamodb_client.exceptions.ConditionalCheckFailedException = boto3.client(
        "dynamodb", region_name="us-east-1"
    ).exceptions.ConditionalCheckFailedException
    mock_dynamodb_client.transact_write_items.side_effect = (
        mock_dynamodb_client.exceptions.TransactionCanceledException(
            error_response={"Error": {"Code": "TransactionCanceledException", "Message": "Failure"}},
            operation_name="TransactWriteItems",
        )
    )

    mock_table.put_item.side_effect = boto3.client(
        "dynamodb", region_name="us-east-1"
    ).exceptions.ConditionalCheckFailedException(
