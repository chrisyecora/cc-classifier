...
def read_transactions() -> list[dict]:
    ...
    """
    table = get_table()
    response = table.scan(FilterExpression=Key("pk").eq(PK_TRX))
    items = response.get("Items", [])

    # Handle pagination if > 1MB (unlikely for personal use but good practice)
    while "LastEvaluatedKey" in response:
        response = table.scan(FilterExpression=Key("pk").eq(PK_TRX), ExclusiveStartKey=response["LastEvaluatedKey"])
...
def delete_transaction(transaction_id: str) -> bool:
    ...
    table = get_table()
    try:
        table.delete_item(Key={"pk": PK_TRX, "sk": transaction_id})
        return True
    except Exception as e:
...
def update_transaction_details(transaction_id: str, amount: str, date_val: str, merchant: str, name: str) -> bool:
    ...
    try:
        table.update_item(
            ...
            ExpressionAttributeValues={":a": amount, ":d": date_val, ":m": merchant, ":n": name},
            ConditionExpression="attribute_exists(pk)",
        )
        return True
    except Exception as e:
...
def read_users() -> dict[str, dict]:
    config = get_config()
    return {"user_a": {"name": config.user_a_name}, "user_b": {"name": config.user_b_name}}
