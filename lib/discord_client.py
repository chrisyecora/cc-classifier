...
def build_classification_components(txn_id: str) -> list:
    ...
    for pct in [25, 30, 35, 40, 60, 65, 70, 75]:
        select_options.append(
            {"label": f"You pay {pct}%", "value": f"S{pct}", "description": f"Split: {pct}/{100 - pct}"}
        )

    select_menu = create_select_menu(
        custom_id=f"classify_split:{txn_id}", options=select_options, placeholder="Custom Split (You pay %)..."
    )

    return [create_action_row(buttons), create_action_row([select_menu])]
...
def send_error_notification(error_message: str) -> bool:
    ...
    # 0xE74C3C is Red
    embed = {"title": "🚨 System Error", "description": f"```{error_message}```", "color": 0xE74C3C}
